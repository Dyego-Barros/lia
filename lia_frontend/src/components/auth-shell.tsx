...
import { ConversationEventsProvider } from "@/components/conversation-events";
...
export function AuthShell({ children }: { children: React.ReactNode }) {
  ...
  return <ConversationEventsProvider><div className="flex min-h-screen min-w-0"><Sidebar /><main className="flex min-h-screen min-w-0 flex-1 flex-col overflow-x-hidden bg-slate-50 p-3 text-slate-900 sm:p-4 md:p-8"><TopNavbar /><div className="min-w-0 flex-1">{children}</div><footer className="mt-10 border-t border-slate-200 py-6 text-center text-xs text-slate-400">© 2026 Mayssa Admin. Todos os direitos reservados. Texto provisório para edição futura.</footer></main></div></ConversationEventsProvider>;
}
