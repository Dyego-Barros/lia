...
type WhatsApp = { id: number; nome: string; tipo: string; prioridade: number; ativo: boolean; ia_ativa: boolean; webhook_configurado: boolean; webhook_url: string; webhook_verify_token?: string | null };
...
export default function ConfiguracoesPage() {
  ...
  const [changingAI, setChangingAI] = useState<number | null>(null);
  ...
  async function toggleWhatsAppAI(item: WhatsApp) {
    if (changingAI !== null) return;
    setChangingAI(item.id);
    try {
      await apiPatch(`/integracoes/whatsapp/${item.id}/ia`, { ativa: !item.ia_ativa });
      setWhatsapp((current) => current.map((integration) => integration.id === item.id ? { ...integration, ia_ativa: !item.ia_ativa } : integration));
      setMessage(`Atendimento automático ${item.ia_ativa ? "desativado" : "ativado"}. O webhook permanece ativo.`);
    } catch (e) {
      setMessage(e instanceof Error ? e.message : "Não foi possível alterar o atendimento automático.");
