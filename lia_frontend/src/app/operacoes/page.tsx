...
type Payment = { id?: number; agendamento_id: number; valor: number; forma: string; status: string; pago_em: string };
...
function OperationsContent() {
  ...
  const [professionals, setProfessionals] = useState<Professional[]>([]); const [blocks, setBlocks] = useState<Block[]>([]); const [waitlist, setWaitlist] = useState<WaitItem[]>([]); const [payments, setPayments] = useState<Payment[]>([]); const [stock, setStock] = useState<Stock[]>([]); const [users, setUsers] = useState<User[]>([]); const [professionalForm, setProfessionalForm] = useState(input); const [blockForm, setBlockForm] = useState({ inicio: "", fim: "", motivo: "" }); const [stockForm, setStockForm] = useState({ nome: "", sku: "", quantidade: "0", estoque_minimo: "0", custo_unitario: "0" }); const [message, setMessage] = useState<string | null>(null);
  ...
  async function submitStock(e: FormEvent) { e.preventDefault(); try { await apiPost("/operacoes/estoque/produtos", { ...stockForm, quantidade: Number(stockForm.quantidade), estoque_minimo: Number(stockForm.estoque_minimo), custo_unitario: Number(stockForm.custo_unitario) }); setStockForm({ nome: "", sku: "", quantidade: "0", estoque_minimo: "0", custo_unitario: "0" }); await load(); setMessage("Produto adicionado ao estoque."); } catch (e) { setMessage(e instanceof Error ? e.message : "Erro ao adicionar produto."); } }
  ...
  const paymentColumns = useMemo<TableColumn<Payment>[]>(() => [{ title: "Agendamento", data: "agendamento_id", render: (row) => `#${row.agendamento_id}` }, { title: "Valor", data: "valor", render: (row) => `R$ ${row.valor.toFixed(2)}` }, { title: "Forma", data: "forma" }, { title: "Status", data: "status" }], []);
  const stockColumns = useMemo<TableColumn<Stock>[]>(() => [{ title: "Produto", data: "nome" }, { title: "SKU", data: "sku", render: (row) => row.sku || "—" }, { title: "Quantidade", data: "quantidade" }, { title: "Mínimo", data: "estoque_minimo" }, { title: "Custo", data: "custo_unitario", render: (row) => `R$ ${row.custo_unitario.toFixed(2)}` }], []);
  return (
    <div className="space-y-6">
      ...
      {section === "estoque" && <>
        <FormCard title="Novo produto no estoque" icon={<Boxes size={18} />} onSubmit={submitStock}>
          ...
          <div className="grid grid-cols-3 gap-2">
            <input type="number" min="0" className="field" placeholder="Qtd." value={stockForm.quantidade} onChange={(e) => setStockForm({ ...stockForm, quantidade: e.target.value })} />
          ...
          <button className="primary-button"><Package size={16} />Adicionar produto</button>
        </FormCard>
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <InteractiveTable data={stock} columns={stockColumns} />
        </section>
      </>}
