...
const statusColors: Record<string, string> = { pendente: "bg-amber-500", confirmado: "bg-indigo-500", concluido: "bg-emerald-500", cancelado: "bg-rose-500", nao_compareceu: "bg-slate-500" };
...
export default function DashboardPage() {
  ...
  const maximumStatus = Math.max(...statusCounts.map(([, count]) => count), 1);
  ...
  const occupancy = professionals.filter((item) => item.ativo && item.id).map((professional) => {
    ...
    return { id: professional.id!, name: professional.nome, count: ownAppointments.length, percentage: capacity ? Math.min((booked / capacity) * 100, 100) : booked ? 100 : 0, next };
  });
  ...
  const cards = [
    ...
    { title: "Total ganho no dia", value: money.format(report?.faturamento ?? 0), hint: "faturamento de hoje", icon: CircleDollarSign },
    { title: "Pagamentos pendentes", value: pendingPayments.length, hint: money.format(expectedPending), icon: CreditCard },
  ];
  ...
  return <div className="space-y-6">
    <section className="rounded-2xl border border-slate-200 bg-white p-7 shadow-sm"><p className="text-sm font-medium uppercase tracking-[0.25em] text-fuchsia-700">MAYA · Dashboard</p><h1 className="mt-2 text-3xl font-semibold text-slate-900">Resumo do dia</h1><p className="mt-2 max-w-3xl text-sm text-slate-500">Agenda, pagamentos e desempenho em uma visão rápida.</p></section>
    {error && <div className="rounded-2xl border border-rose-200 bg-rose-50 p-4 text-sm text-rose-700">{error}</div>}

    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">{cards.map(({ title, value, hint, icon: Icon }) => <div key={title} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-center justify-between"><p className="text-sm text-slate-500">{title}</p><span className="rounded-xl bg-fuchsia-50 p-2 text-fuchsia-700"><Icon size={18} /></span></div><p className="mt-4 text-2xl font-semibold text-slate-900">{value}</p><p className="mt-2 text-sm text-fuchsia-700">{hint}</p></div>)}</div>

    <div className="grid gap-4 xl:grid-cols-3">
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm xl:col-span-2"><div className="flex items-center justify-between"><div><h2 className="flex items-center gap-2 text-lg font-semibold text-slate-900"><CalendarClock size={19} className="text-fuchsia-700" />Próximos atendimentos</h2><p className="mt-1 text-sm text-slate-500">O que ainda está previsto para hoje.</p></div><Link href="/calendario" className="text-sm font-medium text-fuchsia-700">Ver calendário</Link></div>{upcoming.length ? <div className="mt-4 divide-y divide-slate-100">{upcoming.map((item) => <div key={item.id} className="grid gap-2 py-3 text-sm sm:grid-cols-[70px_1fr_1fr_auto] sm:items-center"><strong className="text-slate-900">{item.data_hora.slice(11, 16)}</strong><span className="text-slate-700">{clientNames.get(item.cliente_id) ?? `Cliente #${item.cliente_id}`}</span><span className="text-slate-500">{procedureMap.get(item.procedimento_id)?.nome ?? `Procedimento #${item.procedimento_id}`}</span><span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs text-slate-600">{professionalNames.get(item.profissional_id ?? 0) ?? "Sem profissional"}</span></div>)}</div> : <div className="mt-4 rounded-xl bg-slate-50 p-8 text-center text-sm text-slate-400">Nenhum atendimento restante hoje.</div>}</section>
    ...
    <div className="grid gap-4 lg:grid-cols-3">
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="text-lg font-semibold text-slate-900">Resumo por status</h2><div className="mt-5 space-y-4">{statusCounts.length ? statusCounts.map(([status, count]) => <div key={status}><div className="mb-1.5 flex justify-between text-sm"><span className="text-slate-600">{statusLabels[status] ?? status}</span><strong>{count}</strong></div><div className="h-2.5 overflow-hidden rounded-full bg-slate-100"><div className={`h-full rounded-full ${statusColors[status] ?? "bg-slate-500"}`} style={{ width: `${(count / maximumStatus) * 100}%` }} /></div></div>) : <p className="rounded-xl bg-slate-50 p-6 text-center text-sm text-slate-400">Sem agendamentos hoje.</p>}</div></section>
    ...
    <div className="grid gap-4 xl:grid-cols-2">
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="flex items-center gap-2 text-lg font-semibold text-slate-900"><UserCheck size={19} className="text-fuchsia-700" />Ocupação das profissionais</h2><p className="mt-1 text-sm text-slate-500">Tempo agendado em relação à escala de hoje.</p><div className="mt-5 space-y-5">{occupancy.length ? occupancy.map((item) => <div key={item.id}><div className="mb-2 flex items-end justify-between gap-3"><div><strong className="text-sm text-slate-800">{item.name}</strong><p className="text-xs text-slate-400">{item.count} atendimento(s){item.next ? ` · próximo ${item.next.data_hora.slice(11, 16)}` : ""}</p></div><span className="text-sm font-semibold text-slate-700">{item.percentage.toFixed(0)}%</span></div><div className="h-3 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-gradient-to-r from-fuchsia-600 to-emerald-400" style={{ width: `${item.percentage}%` }} /></div></div>) : <p className="rounded-xl bg-slate-50 p-6 text-center text-sm text-slate-400">Nenhuma profissional ativa.</p>}</div></section>
