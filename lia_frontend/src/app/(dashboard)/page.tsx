...
export default function DashboardPage() {
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-4 xl:grid-cols-2">
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="flex items-center gap-2 text-lg font-semibold text-slate-900"><UserCheck size={19} className="text-fuchsia-700" />Ocupação das profissionais</h2><p className="mt-1 text-sm text-slate-500">Tempo agendado em relação à escala de hoje.</p>{occupancy.length ? <div className="mt-6 grid gap-4 sm:grid-cols-2">{occupancy.map((item) => <div key={item.id} className="flex items-center gap-4 rounded-2xl bg-slate-50 p-4"><div className="relative h-24 w-24 shrink-0 rounded-full" style={{ background: `conic-gradient(#c026d3 0 ${item.percentage}%, #e2e8f0 ${item.percentage}% 100%)` }}><div className="absolute inset-3 flex items-center justify-center rounded-full bg-white shadow-inner"><strong className="text-lg text-slate-900">{item.percentage.toFixed(0)}%</strong></div></div><div className="min-w-0 text-left"><strong className="block truncate text-sm text-slate-800">{item.name}</strong><p className="mt-1 text-xs text-slate-500">{item.count} atendimento(s)</p>{item.next && <p className="mt-1 text-xs font-medium text-fuchsia-700">Próximo às {item.next.data_hora.slice(11, 16)}</p>}</div></div>)}</div> : <p className="mt-5 rounded-xl bg-slate-50 p-6 text-center text-sm text-slate-400">Nenhuma profissional ativa.</p>}</section>
