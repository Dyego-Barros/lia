...
from app.api.routes.dependencies import agendamento_repository
...
async def criar(payload: AgendamentoCreate, repository: AgendamentoRepository = Depends(agendamento_repository)):
    try: return await CadastrarAgendamento(AgendamentoService(repository)).execute(AgendamentoDto(**payload.model_dump()))
...
async def atualizar(agendamento_id: int, payload: AgendamentoCreate, repository: AgendamentoRepository = Depends(agendamento_repository)):
    try: return await AtualizarAgendamento(AgendamentoService(repository)).execute(AgendamentoDto(id=agendamento_id, **payload.model_dump()))
    except ValueError as exc: raise HTTPException(404, str(exc)) from exc
